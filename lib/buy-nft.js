...
$(document).ready(() => {
    ...
    formdata.append("referrer", reffererTag)
      ...
      redirect: 'follow'
        ...
        apiresponse = result
          ...
          $(".down-payment").text(`${downpayment.toFixed(4)} wETH`)
          ...
          $(".finalized-nft-image").attr("src", result.data.addnlContextData.imageUrl)
				} else {
          $("#final-steps").show()
  ...
  $("#buynow").click(async () => {
    ...
    try {
      ...
      let downPayment = new ethers.BigNumber.from(apiresponse.data.submitBidArgs.downPayment)
      ...
      if (!apiresponse.data.craSignature) {
        ...
        // If WETH balance is less than downpayment, deposit the difference in ETH into WETH contract
        if (wethBalance.lt(downPayment)) {
          await wethContract.deposit({
            gasLimit: "300000",
            value: downPayment.sub(wethBalance)
          })
        }

        const allowance = await wethContract.allowance(signerAddress, bnplAddress)

        // Approve BNPLMarket for the downpayment
        if (allowance.lt(downPayment)) {
          await wethContract.approve(bnplAddress, downPayment)
        }
