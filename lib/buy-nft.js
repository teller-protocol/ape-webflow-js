...
$(document).ready(() => {
	...
	$(window).keydown((event) => {
		...
		ethereum.enable()
			.then(async() => {
				...
				const shortAddress=`${signerAddress.slice(0, 4)}...${signerAddress.slice(signerAddress.length - 3, signerAddress.length)}`
				$("#wallet-btn").text(shortAddress)
				...
				$("#NFT-Collection-field").change(async() => await getNFTData(true))
				$("#token-id").keyup(async() => await getNFTData())
			})
	...
	function fetchNFT(contractAddr, tokenId) {
		var reqOptions = {method: 'GET',redirect: 'follow'};
		...
		fetch(fetchURL, reqOptions)
			...
			console.log("name", collections[name.slice(0, 4)], name)
		...
		var nftAvailabilityReqOptions = {method:'POST',body:formdata,redirect:'follow'};
		const nftURL = `https://bd93-2405-201-c01d-f822-28bc-f015-93c2-b5c1.in.ngrok.io/api/checkNftAvailabilityOnOpenSea?`
		fetch(nftURL, nftAvailabilityReqOptions)
			.then(response => response.json())
